...
function Sidebar({ grouped, handleNavClick }) {
    return (
      <aside className="w-84 sticky top-0 h-screen overflow-y-auto border-r border-slate-700 p-4 flex flex-col">
        <nav>
              ...
              <ul className="border-l border-gray-300 pl-4 mb-4 font-crimson-pro">
                {list.map(page => (
                  <li key={page.id} className="mb-2">
                    <a href={`#${page.slug}`} onClick={(e) => handleNavClick(e, page)} className="text-gray-300 hover:text-white transition font-nunito-sans">{page.title}</a>
                  </li>
                ))}
              </ul>
            </div>
